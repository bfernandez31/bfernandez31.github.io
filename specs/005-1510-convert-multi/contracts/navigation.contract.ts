...
export function validateNavigationMenu(menu: NavigationMenu): void {
	...
	if (menu.links.length !== 5) {
		throw new Error(
			`NavigationMenu must have exactly 5 links (one per section). Got: ${menu.links.length}`,
		);
	}
	...
	menu.links.forEach((link, index) => {
		try {
			...
			throw new Error(
				`NavigationMenu link at index ${index} is invalid: ${message}`,
			);
		}
	});
	...
	const activeCount = menu.links.filter(
		(link) => link.ariaCurrent === "page",
	).length;
	if (activeCount > 1) {
		throw new Error(
			'NavigationMenu can have only one active link (ariaCurrent="page")',
		);
	}
