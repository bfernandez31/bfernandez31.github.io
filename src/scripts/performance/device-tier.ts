...
export function detectDeviceTier(): DeviceTierResult {
  ...
  if (cores === undefined && memoryGB === undefined) {
    reasons.push('Device APIs unavailable (privacy browser or unsupported browser)');
    return {
      tier: 'UNKNOWN',
      reasons,
      capabilities,
  ...
  if (
    ...
    reasons.push(`CPU cores ≥ 4 AND Memory ≥ 4GB (${cores} cores, ${memoryGB}GB detected)`);
    return {
      tier: 'HIGH',
      reasons,
      capabilities,
  ...
  if (isMidRangeHardware) {
    ...
    if (effectiveType && slowConnections.includes(effectiveType)) {
      reasons.push(
        `Mid-range hardware but slow connection (${effectiveType})`
      );
